...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT Cust_ID FROM customers WHERE Cust_ID = ?";
    ...
    if(empty(trim($_POST["fname"]))){
        ...
        // Prepare a select statement
        $sql = "SELECT Fname FROM customers WHERE Fname = ?";
    ...
    if(empty(trim($_POST["lname"]))){
        ...
        // Prepare a select statement
        $sql = "SELECT Lname FROM customers WHERE Lname = ?";
    ...
    if(empty(trim($_POST["loc"]))){
        ...
        // Prepare a select statement
        $sql = "SELECT location FROM customers WHERE location = ?";
  ...
  /*  if(empty(trim($_POST["confirm_password"]))){
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty ($loc_err) && empty($fname_err) && empty($lname_err)){

        // Prepare an insert statement
        $sql = "INSERT INTO customers (Cust_ID, Fname, Lname, Password, email, Location) VALUES (?, ?, ?, ?,?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_fname, $param_lname, $param_password, $param_email, $param_loc);
  ...
  <div class="container">
    ...
    <h1 style="color: blue ; text-align: center; padding-bottom: 15px;">User Registration</h1>
     ...
     <form class="row g-3" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
       ...
       <!-- Address -->
       <div class="col-12 form-group <?php echo (!empty($loc_err)) ? 'has-error' : ''; ?>">
         <label for="inputAddress" class="form-label">Location</label>
         <input name="loc" type="text" class="form-control" id="inputAddress" placeholder="e.g, Dhanmondi, Bashundhara" value="<?php echo $loc; ?>">
       </div>
